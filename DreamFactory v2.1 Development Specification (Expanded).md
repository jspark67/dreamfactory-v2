# **DreamFactory v2.1 Development Specification (Expanded)**
...
#### **Prompt 4: FastAPI 엔드포인트 및 비동기 처리 연결**

\*\*Role:\*\* Full Stack Developer

\*\*Objective:\*\* Create API endpoints in FastAPI to trigger agents asynchronously.

\*\*Instructions:\*\*  
1\.  \*\*Update \`main.py\`:\*\*  
2\.  \*\*Define Request Models:\*\* Create Pydantic models for requests (e.g., \`GenerateVideoRequest\` with \`scene\_id\`, \`image\_url\`, \`prompt\`).  
3\.  \*\*Create Endpoints:\*\*  
    \* \`POST /generate/video\`: Accepts \`GenerateVideoRequest\`.  
    \* \*\*Background Task:\*\* Use \`FastAPI.BackgroundTasks\` to call \`director.generate\_video\` so the API returns immediately with \`{ "status": "queued" }\`.  
4\.  \*\*Next.js Integration:\*\* (Instruction for Frontend)  
    \* Modify Next.js \`src/app/api/orchestrator/route.ts\` to \`fetch\` this Python endpoint instead of calling local TS agents for the Director step.

\*\*Goal:\*\* Non-blocking UI where the user clicks "Generate" and the UI updates later via Firestore listener.  
