...
// Mock Video type since we don't have @google/genai
type Video = unknown;
...
const modeIcons: Record<GenerationMode, React.ReactNode> = {
    ...
    [GenerationMode.TEXT_TO_IMAGE]: <Wand2Icon className="w-5 h-5" />,
};
...
const CustomSelect: React.FC<{
    ...
    <div>
        <label
            className={`text-xs block mb-1.5 font-medium ${disabled ? 'text-white/40' : 'text-white/70'
                }`}>
            {label}
        </label>
        <div className="relative">
            ...
            <select
                ...
                onChange={onChange}
                disabled={disabled}
                className="w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-8 py-2.5 appearance-none focus:ring-2 focus:ring-white/20 focus:border-white/20 disabled:bg-white/5 disabled:border-white/5 disabled:text-white/30 disabled:cursor-not-allowed text-white">
                {children}
            </select>
            <ChevronDownIcon
                className={`w-5 h-5 absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none ${disabled ? 'text-white/30' : 'text-white/60'
                    }`}
            />
        </div>
    </div>
...
const ImageUpload: React.FC<{
    ...
    return (
        <button
            ...
            className="w-28 h-20 bg-white/5 hover:bg-white/10 border-2 border-dashed border-white/20 rounded-lg flex flex-col items-center justify-center text-white/60 hover:text-white transition-colors">
...
const VideoUpload: React.FC<{
    ...
    return (
        <button
            type="button"
            onClick={() => inputRef.current?.click()}
            className="w-48 h-28 bg-white/5 hover:bg-white/10 border-2 border-dashed border-white/20 rounded-lg flex flex-col items-center justify-center text-white/60 hover:text-white transition-colors text-center">
            <PlusIcon className="w-6 h-6" />
...
const PromptForm: React.FC<PromptFormProps> = ({
    ...
    const renderMediaUploads = () => {
        if (generationMode === GenerationMode.FRAMES_TO_VIDEO) {
            return (
                <div className="mb-3 p-4 bg-white/5 rounded-xl border border-white/10 flex flex-col items-center justify-center gap-4">
                    ...
                    {startFrame && !endFrame && (
                        <div className="mt-3 flex items-center">
                            ...
                            <label
                                ...
                                className="ml-2 text-sm font-medium text-white/70 cursor-pointer">
        ...
        if (generationMode === GenerationMode.REFERENCES_TO_VIDEO) {
            return (
                <div className="mb-3 p-4 bg-white/5 rounded-xl border border-white/10 flex flex-wrap items-center justify-center gap-2">
        ...
        if (generationMode === GenerationMode.EXTEND_VIDEO) {
            return (
                <div className="mb-3 p-4 bg-white/5 rounded-xl border border-white/10 flex items-center justify-center gap-4">
                    <VideoUpload
                        label={
                            <>
                                Input Video
    ...
    return (
        <div className="relative w-full">
            {isSettingsOpen && (
                <div className="absolute bottom-full left-0 right-0 mb-3 p-4 bg-[#1a1a1a] rounded-xl border border-white/10 shadow-xl text-white">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <CustomSelect
                            label="Model"
                            value={model}
                            onChange={(e) => setModel(e.target.value as VeoModel)}
                            icon={<SparklesIcon className="w-5 h-5 text-white/60" />}
                            disabled={isRefMode || isImageMode}>
                        ...
                        <CustomSelect
                            ...
                            icon={<RectangleStackIcon className="w-5 h-5 text-white/60" />}
                        ...
                        <div>
                            <CustomSelect
                                ...
                                icon={<TvIcon className="w-5 h-5 text-white/60" />}
                                disabled={isRefMode || isExtendMode || isImageMode}>
            ...
            <form onSubmit={handleSubmit} className="w-full">
                {renderMediaUploads()}
                <div className="flex items-end gap-2 bg-white/5 border border-white/10 rounded-2xl p-2 shadow-sm focus-within:ring-2 focus-within:ring-white/20 focus-within:border-white/20">
                    <div className="relative" ref={modeSelectorRef}>
                        <button
                            type="button"
                            onClick={() => setIsModeSelectorOpen((prev) => !prev)}
                            className="flex shrink-0 items-center gap-2 px-3 py-2.5 rounded-lg hover:bg-white/10 text-white/80 hover:text-white transition-colors"
                        ...
                        {isModeSelectorOpen && (
                            <div className="absolute bottom-full mb-2 w-60 bg-[#1a1a1a] border border-white/10 rounded-lg shadow-xl overflow-hidden z-10 text-white">
                                {selectableModes.map((mode) => (
                                    <button
                                        key={mode}
                                        type="button"
                                        onClick={() => handleSelectMode(mode)}
                                        className={`w-full text-left flex items-center gap-3 p-3 hover:bg-white/10 ${generationMode === mode ? 'bg-white/10 text-white font-medium' : 'text-white/70'}`}>
                                        {modeIcons[mode]}
                                        <span>{mode}</span>
                                    </button>
                                ))}
                    ...
                    <textarea
                        ...
                        className="flex-grow bg-transparent focus:outline-none resize-none text-base text-white placeholder-white/40 max-h-48 py-2 min-h-[40px]"
                        rows={1}
                    />
                    <button
                        type="button"
                        onClick={() => setIsSettingsOpen((prev) => !prev)}
                        className={`p-2.5 rounded-full hover:bg-white/10 ${isSettingsOpen ? 'bg-white/10 text-white' : 'text-white/60'}`}
                    ...
                    <div className="relative group">
                        <button
                            type="submit"
                            className="p-2.5 bg-white rounded-full hover:bg-gray-200 disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors"
                            aria-label="Generate"
